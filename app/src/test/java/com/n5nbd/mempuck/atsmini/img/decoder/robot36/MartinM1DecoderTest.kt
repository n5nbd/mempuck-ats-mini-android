...
class MartinM1DecoderTest {
    @Test
    fun correctsCallbackFrameAliasedSyncPositions() {
        val expected = 1_112_010L
        assertEquals(
            expected - 3,
            MartinM1Decoder.correctFrameAliasedSyncSample(
                expected - 885,
                expected,
                882,
                264,
    ...
    @Test
    fun acceptsMartinM1VisWhenFirstLeaderWindowIsDegraded() {
        val audio = MartinM1SignalBuilder(sampleRateHz = 44_100).apply {
            tone(1_900.0, 0.100)
            visHeader(
                code = 44,
        ...
        val decoder = MartinM1Decoder(
            44_100,
            object : MartinM1Decoder.Listener {
        ...
        assertEquals("MARTIN M1", detectedMode)
        ...
        assertTrue(diagnostics.any { it.startsWith("VIS accepted code=44") })
    }

    @Test
    fun detectsMartinM1VisAndRendersProgressiveColorLines() {
        val audio = MartinM1SignalBuilder(sampleRateHz = 44_100).apply {
            tone(1_900.0, 0.100)
            visHeader(code = 44)
            repeat(8) { line ->
                when (line % 3) {
                    0 -> martinM1Line(redHz = 2_250.0, greenHz = 1_650.0, blueHz = 1_550.0)
                    1 -> martinM1Line(redHz = 1_550.0, greenHz = 2_250.0, blueHz = 1_650.0)
                    else -> martinM1Line(redHz = 1_650.0, greenHz = 1_550.0, blueHz = 2_250.0)
                }
            }
        }.build()
        ...
        assertTrue(diagnostics.any { it.startsWith("VIS accepted code=44") })
        assertTrue(diagnostics.any { it.startsWith("MARTIN1 line_anchor_consumed") })
        assertTrue(lastFrame?.any { it != 0xff000000.toInt() } == true)
    }
}

private class MartinM1SignalBuilder(
    ...
    fun martinM1Line(
        ...
        tone(greenHz + frequencyOffsetHz, 0.146432)
        tone(1_500.0 + frequencyOffsetHz, 0.000572)
        tone(blueHz + frequencyOffsetHz, 0.146432)
        tone(1_500.0 + frequencyOffsetHz, 0.000572)
        tone(redHz + frequencyOffsetHz, 0.146432)
        tone(1_500.0 + frequencyOffsetHz, 0.000572)
    }
