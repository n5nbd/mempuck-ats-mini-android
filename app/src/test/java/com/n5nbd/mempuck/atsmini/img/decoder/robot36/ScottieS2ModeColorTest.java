...
public class ScottieS2ModeColorTest {
    private static final int SAMPLE_RATE = 44_100;
    private static final int WIDTH = 320;

    @Test
    public void decodesScottieGreenBlueSyncRedChannelOrder() {
        ScottieS2Mode mode = new ScottieS2Mode(SAMPLE_RATE);
    ...
    private static float[] scanLine(
        ...
        int channel = (int) Math.round(0.088064 * SAMPLE_RATE);
        int greenBegin = syncIndex - (int) Math.round(0.186628 * SAMPLE_RATE);
        int blueBegin = syncIndex - (int) Math.round(0.097064 * SAMPLE_RATE);
