...
class ScottieS2DecoderTest {
    @Test
    fun correctsCallbackFrameAliasedSyncPositions() {
        val expected = 1_112_010L
        assertEquals(
            expected - 3,
            ScottieS2Decoder.correctFrameAliasedSyncSample(
                expected - 885,
                expected,
                882,
                264,
    ...
    @Test
    fun detectsScottieS2VisSkipsStartingSyncAndRendersProgressiveColorLines() {
        val audio = ScottieS2SignalBuilder(sampleRateHz = 44_100).apply {
            tone(1_900.0, 0.100)
            visHeader(code = 56)
            scottieImage(8) { line ->
                when (line % 3) {
        ...
        val decoder = ScottieS2Decoder(
            44_100,
            object : ScottieS2Decoder.Listener {
        ...
        assertEquals("SCOTTIE S2", detectedMode)
        assertTrue("Expected progressive Scottie line output", completedLines >= 4)
        assertTrue(diagnostics.any { it.startsWith("VIS accepted code=56") })
        assertTrue(diagnostics.any { it.startsWith("SCOTTIE2 first_sync_acquired") })
        assertTrue(diagnostics.any { it.startsWith("SCOTTIE2 line_anchor_consumed") })
        assertTrue(lastFrame?.any { it != 0xff000000.toInt() } == true)
    }

    @Test
    fun rejectsScottieS1VisCode() {
        val audio = ScottieS2SignalBuilder(sampleRateHz = 44_100).apply {
            tone(1_900.0, 0.100)
            visHeader(code = 60)
    ...
    @Test
    fun manualScottieS2StartsWithoutVisHeader() {
        val audio = ScottieS2SignalBuilder(sampleRateHz = 44_100).apply {
            scottieImage(
                lines = 12,
        ...
        assertTrue(detectedMode?.contains("SCOTTIE S2 RAW") == true)
        assertTrue("Expected manual S2 partial lines", completedLines > 0)
    }

    @Test
    fun completesFullScottieS2Frame() {
        val audio = ScottieS2SignalBuilder(sampleRateHz = 44_100).apply {
            tone(1_900.0, 0.100)
            visHeader(code = 56)
...
private class ScottieS2SignalBuilder(
    ...
    fun scottieImage(
        ...
        repeat(lines) { line ->
            ...
            tone(1_500.0, 0.0015)
            tone(greenHz, 0.088064)
            tone(1_500.0, 0.0015)
            tone(blueHz, 0.088064)
            tone(1_200.0, 0.009)
            tone(1_500.0, 0.0015)
            tone(redHz, 0.088064)
        }
